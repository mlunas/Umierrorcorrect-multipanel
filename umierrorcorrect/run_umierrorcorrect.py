...
:Author: Tobias Osterlund & Manuel Luna

Purpose
-------

Run the custom pipeline for dual panels on same index
...
from umierrorcorrect.split_bam import split_bam
import argparse
import os
import logging
...
def parseArgs():
    ...
    group1.add_argument('-b2','--bed2', dest='bed2', 
                        help='Path to a  second BED file used in an alternative workflow separating reads into 2 BAM files after alignment.')
...
def main(args):
    ...
    if not args.bed2:
        ...
        bam_A, bam_B = split_bam(args.output_path, args.sample_name, args.bam_file, args.bed_file, args.bed2)
        sample_og=args.sample_name
        for bam in bam_A, bam_B:
            args.bam_file = bam
            if bam==bam_B:
                print("Processing BAM B")
                args.sample_name=sample_og+"_B"
                args.bed_file = args.bed2
            else:
                args.sample_name=sample_og+"_A"
                print("Processing BAM A")
            run_umi_errorcorrect(args)  #run umi errorcorrect
            cons_bam = args.output_path + '/' + args.sample_name + '_consensus_reads.bam'
            stat_filename = args.output_path + '/' + args.sample_name + '.hist'
            run_get_consensus_statistics(args.output_path, cons_bam, stat_filename,False, args.sample_name)
            args.cons_file=args.output_path + '/' + args.sample_name + '_cons.tsv'
            #args.params_file=None
            run_call_variants(args)
        logging.info("Finished UMI Error Correct")
